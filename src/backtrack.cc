 ...
 * Contents :: Implementation of DAF algorithm to find subgraph matching problem
 * Date :: 2021-06-08
*/
...
void get_match_order(const Graph &data, const Graph &query, const CandidateSet &cs, std::vector<std::vector<int>> &DAG, std::vector<std::vector<int>> &path, int current) {
    ...
    // Put current vector's child's number of child vertex to num_candidates vector.
    ...
    // Sorting index vector in order of child size. (index 0 has the smallest number of child vertexes.) 
